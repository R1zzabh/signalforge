...
def enrich(a):
    ...
    return {'virustotal':{'provider':'VirusTotal','status':'demo' if DEMO_MODE else 'unavailable','detections':14 if malicious else 0,'total_engines':70,'last_lookup':now()},'abuseipdb':{'provider':'AbuseIPDB','status':'demo' if DEMO_MODE else 'unavailable','confidence':94 if malicious else 4,'country':'NL' if malicious else 'US','usage_type':'hosting infrastructure' if malicious else 'residential','last_lookup':now()},'geoip':{'provider':'GeoIP','status':'demo' if DEMO_MODE else 'unavailable','country':'Netherlands' if malicious else 'United States','city':'Amsterdam' if malicious else 'Ashburn','asn':'AS9009 M247 Europe' if malicious else 'AS15169 Google'},'nvd':{'provider':'NVD/CVE','status':'demo' if DEMO_MODE else 'unavailable','cvss':9.8 if cve else 0,'cve':cve,'component':'OpenSSH' if cve else None,'affected_version':'8.2' if cve else None}}
...
def investigate(alert, scenario=None):
    ...
    ai=summary(alert,ti,risk); stage('AI summary generated','ai','Local deterministic explanation generated' if DEMO_MODE else 'Provider unavailable; fallback explanation generated')
